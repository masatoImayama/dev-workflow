...
**組み立ては `scripts/format-lane-result.sh` に切り出してある**（Task #156。決定論的な
文字列処理をLLMで毎回組み立て直さない。`plan-waves.sh` / `merge-lane.sh` と同じ発想）。
各 generator の報告に含まれる `evidence=` のパス（`core/roles/generator.md`「完了報告」節）を
レーンごとに集め、そのレーンが処理したタスクの順で `--file` に渡す:

```bash
bash "${CLAUDE_PLUGIN_ROOT}/scripts/format-lane-result.sh" --lane A --file "[レーンAの1件目のevidence]" --file "[レーンAの2件目のevidence]"
# => A=#5,#11(12:03-12:16 12m00s)
```

これをレーンごとに呼び、`レーン結果: ` に続けて空白区切りで並べる。失敗したタスクは
出力側で自動的に `#<task>(失敗)` と付記される（証跡ファイルの `status=` が `success` 以外の場合）。
証跡ファイルが見つからない・必須フィールドが欠けている場合は exit 2 で止まるので、
その旨をそのまま表示し、該当レーンの表示は「詳細不明」として進める（自律ループ自体は止めない）。
