...
- **1件のタスクに失敗しても、レーン全体を投げ出さない。** 失敗したタスクは `git restore --source=HEAD --staged --worktree .` で追跡ファイルをHEADの状態へ戻し、`git clean -nd`（dry-run。削除はしない）で残る未追跡ファイルを報告してから見送り、次のタスクへ進む（`git reset --hard` はベース合わせと同じ理由で`permissions.deny`にブロックされうるため、ここでも使わない）
