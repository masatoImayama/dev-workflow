...
**差分に現れないファイルの中身（周辺コード・設定ファイル等）を確認したい場合は、シェルを
経由しない読み取り・検索手段があればそれを優先する。** `$` や空白を含むファイル名
（コード生成ツールの出力等）をシェル経由（`cat` / `sed` / `head` / `grep` 等）で読もうと
すると、シェル展開の承認プロンプトを誘発しうる（issue #140）。`git diff` / `git log` 自体は
シェルのコマンド実行手段で実行してよい（差分取得はこの制約の対象外）。
