...
**自己申告ではなく、コマンドの実出力を貼ること。** 「ベースは正しい」「問題なし」といった
結論だけの報告は、実態と食い違っていても検出できない。

報告は次の3部構成にする。

1. **レーン先頭のベース検証**（レーンにつき1回だけ）
2. **タスクごとのブロック**（割り当てられたタスクの数だけ繰り返す。見送ったタスクも書く）
3. **レーン末尾のサマリ**（レーンにつき1回だけ）
...
### 準備（準備コマンド／共有ディレクトリが渡された場合のみ）
$ bash .../share-prepared-dirs.sh ...
[実出力の各行と prep= 行をそのまま]
```

### 2. タスクごとのブロック（割り当ての数だけ繰り返す）

```
## Task #[番号] 完了

### 所要時間
開始: [date +%s の値 または HH:MM] / 終了: [同左]
